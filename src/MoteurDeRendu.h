...
public:
    ...
    void sauvegarderDocumentXml(const char* nom_fichier) ;
...
void MoteurRendu::sauvegarderDocumentXml(const char* nom_fichier) {
    tinyxml2::XMLDocument xmlDoc;
...
couleur couleur_rayon(const rayon& r,  ObjectList& monde, int profondeur) {
    ...
    if (monde.intersect(r, 0.001, infinity, rec)) {
        rayon interactionR;
        couleur attenuation;
    ...
    vecteur3 direction_unite = vecteur_unitaire(r.direction());
    ...
    if (en_travail) {
        ...
        for (int j = hauteur_img-1; j >= 0; --j) {
            ...
            for (int i = 0; i < largeur_img; ++i) {
                ...
                entrer_couleur(pixels, couleur_pixel, echantillons_par_pixel, (hauteur_img-1) - j, i, largeur_img);
            }
        }
        en_travail = false;
        image_pret = true;
