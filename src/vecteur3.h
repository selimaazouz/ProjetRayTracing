...
public:
 ...
 vecteur3& operator=(const vecteur3& w) {
    ...
    inline static vecteur3 random(double mini, double maxi) {
        return vecteur3(random_double(mini, maxi), random_double(mini, maxi), random_double(mini, maxi));
    }
...
std::ostream& operator<<(std::ostream& flux, const vecteur3& w) {
    return flux << w.v[0] << ' ' << w.v[1] << ' ' << w.v[2];
}

vecteur3 operator+(const vecteur3& u, const vecteur3& w) {
    return vecteur3(u.v[0] + w.v[0], u.v[1] + w.v[1], u.v[2] + w.v[2]);
}
...
vecteur3 operator*(double t, const vecteur3& w) {
    return vecteur3(t * w.v[0], t * w.v[1], t * w.v[2]);
}
...
vecteur3 produit_vectoriel(const vecteur3& u, const vecteur3& w) {
    return vecteur3(u.v[1] * w.v[2] - u.v[2] * w.v[1], u.v[2] * w.v[0] - u.v[0] * w.v[2], u.v[0] * w.v[1] - u.v[1] * w.v[0]);
}
...
vecteur3 point_aleatoire_dans_disque() {
    while (true) {
        auto p = vecteur3(random_double(-1, 1), random_double(-1, 1), 0);
        if (p.norme2() < 1)
            return p;
    }
}
