...
class materiau {
    public:
         virtual bool interaction(
            const rayon& r,  EnregIntersect& rec, couleur& attenuation, rayon& interactionR
        ) const = 0;
...
class LambertianMateriau : public materiau {
    public:
      ...
      virtual bool interaction(
            const rayon& r,  EnregIntersect& rec, couleur& attenuation, rayon& intercationR
        ) const override {
...
class MetalMateriau : public materiau {
    public:
       ...
       virtual  bool interaction(
            ...
            vecteur3 reflected = refleter(vecteur_unitaire(r.direction()), rec.surface_normal);
            intercationR = rayon(rec.p, reflected + reflectionfuzz*point_aleatoire_dans_sphere(), r.temps());
            attenuation = diffuseCouleur;
...
class DielectricMateriau : public materiau {
    public:
      ...
      virtual bool interaction(
            ...
            double refraction_ratio = rec.is_front_face? (1.0/indexRefraction ) : indexRefraction ;
            ...
            if (cannot_refract || reflectance(cos, refraction_ratio) > random_double())
                direction = refleter(unit_direction, rec.surface_normal);
            else
                direction = refracter(unit_direction, rec.surface_normal, refraction_ratio);
